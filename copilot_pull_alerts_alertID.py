...
ALERT_LIMIT = 10          # berapa alert unlinked yang ditarik
CHILD_LIMIT = 200          # limit per tabel child (history/event/comment/etc)
ASSET_LIMIT = 200          # limit asset per alert
ASSET_CONTEXT_LIMIT = 200  # limit context per alert (via asset.alert_context_id)
ORDER_BY = "a.`alert_creation_time` DESC"  # fallback kalau kolom ada
# ================================================
...
def fetch_assets_for_alert(cur, alert_id: int):
    """
    Relasi asset yang "pasti" dari DDL kamu:
      incident_management_asset.alert_linked -> incident_management_alert.id
      incident_management_asset.alert_context_id -> incident_management_alertcontext.id
    """
...
def fetch_unlinked_alert_ids(cur):
    # alert yang tidak punya row di casealertlink
    cur.execute(
        f"""
        SELECT a.`id`
        FROM `incident_management_alert` a
        LEFT JOIN `incident_management_casealertlink` cal
          ON cal.`alert_id` = a.`id`
        WHERE cal.`alert_id` IS NULL
        ORDER BY {ORDER_BY if has_column(cur, "incident_management_alert", "alert_creation_time") else "a.`id` DESC"}
        LIMIT %s
        """,
        (ALERT_LIMIT,),
    )
...
def main():
    try:
        # Terima Alert ID dari command-line argument atau user input
        alert_id = None
        ...
        if len(sys.argv) > 1:
            try:
                ...
                print(f"ERROR: Alert ID harus berupa angka. Anda input: {sys.argv[1]}", file=sys.stderr)
                sys.exit(1)
        else:
            try:
                alert_id = int(input("Masukkan Alert ID: "))
            except ValueError:
                print("ERROR: Alert ID harus berupa angka.", file=sys.stderr)
                sys.exit(1)
        
        conn = connect()
        cur = conn.cursor()

        b = bundle_alert(cur, alert_id)
        
        if not b:
            print(f"ERROR: Alert ID {alert_id} tidak ditemukan di database.", file=sys.stderr)
            cur.close()
            conn.close()
            sys.exit(1)
